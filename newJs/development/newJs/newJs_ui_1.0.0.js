...
newJs_ui.Buoy = function(ele) {
	...
	if (ele.id && win.localStorage && (strStoreDistance = localStorage['newJs_ui_' + ele.id])) {
		...
		var step = function() {
			...
			if (speed < 0.1) {
				...
				if (params.edge == false) {
					...
					if (win.localStorage) {
						localStorage['newJs_ui_' + ele.id] = [x, y].join();
					}
				} else {
					// 边缘吸附
					edge();
